...
function [scanConvertedImage, Xs, Zs] = getScanConvertedImage(inputImage, thetas, ranges, sizeX, sizeZ, interpolationMethod)
...
if isvector(inputImage)
    inputImage = reshape(inputImage, length(ranges), length(thetas), []);
%     reshape(b_data_mv_getCapon.data,512,128,[])
...
% % Normalizes and replaces NaN with 1. (Added to remove work within the
% % scripts)
% scanConvertedImage = scanConvertedImage(:,:)/max(max(scanConvertedImage(:,:)));
