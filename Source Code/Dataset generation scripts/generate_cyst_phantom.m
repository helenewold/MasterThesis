...
%     cd('/uio/hume/student-u55/helenewo/MasterThesis/MasterThesis/Source Code')
end

filename = 'phantom_cyst.uff';
data_path = fullfile('/uio/hume/studentg-u55/helenewo/MasterThesis/datasets');
...
xxp_speckle=(random('unif',-30e-3,30e-3,number_of_scatterers,1));
...
tmpx = find(abs(sca(:,1))<5e-3);
tmpz1 = find(abs(sca(:,3))<35e-3);
tmpz2 = find(abs(sca(:,3))>25e-3);
tmpz = intersect(tmpz1, tmpz2);
indtmp = intersect(tmpx, tmpz);
% 
% count = 0;
% while ~isempty(indtmp)
%     ntmpscat = length(indtmp);
%     sca(indtmp,:) = [random('unif',-10e-3,10e-3,ntmpscat,1) zeros(ntmpscat,1) random('unif',20e-3,40e-3,ntmpscat,1)];
%     tmpx = find(abs(sca(:,1))<5e-3);
%     tmpz1 = find(abs(sca(:,3))<35e-3);
%     tmpz2 = find(abs(sca(:,3))>25e-3);
%     tmpz = intersect(tmpz1, tmpz2);
...
%  6 mm cyst
r=5/1000;      %  Radius of cyst [mm]
xc=0/1000;     %  Place of cyst [mm]
zc=30/1000;%+z_start;  

amp=randn(length(sca),1);                   % list with the scatterers amplitudes

inside = ( ((xxp_speckle-xc).^2 + (zzp_speckle-zc).^2) < r^2)
amp = amp .* (1-inside); 


% for i = 1:number_of_scatterers
% %     C=intersect(circle,sca);
%     for j = 1:number_of_scatterers
%         if abs(sca(i,:)-circle(j,:))<1e-4
%             sca(i,:) = [0 0 0]
%             i
%         end
%     end
...
% amp(indtmp) = zeros(length(indtmp),1)
