...
%% phantom of speckle
number_of_scatterers = 1e4;
xxp_speckle=random('unif',-25e-3,25e-3,number_of_scatterers,1);
zzp_speckle=random('unif',2e-3,58e-3,number_of_scatterers,1);
...
for n=1:probe.N
    ...
    x_focus = sind(transmit_angles(n)).*60/1000;
    z_focus = cosd(transmit_angles(n)).*60/1000;
    % transmit aperture
...
scan = uff.sector_scan('azimuth_axis',linspace(azimuth_axis(1), azimuth_axis(end), length(azimuth_axis))' ,'depth_axis',depth_axis);
...
%% Save UFF dataset
filename=[ustb_path(),'/data/FieldII_speckle_simulation_NIGHTRUN.uff'];
channel_data.write(filename,'channel_data');
b_data.write(filename,'beamformed_data');

b_data.plot(figure(10))