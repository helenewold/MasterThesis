...
filename = 'speckle_lambda1.uff';
data_path = fullfile('datasets', 'lambda1');
...
noSubAz=round(probe.element_width/(lambda/1));        % number of subelements in the azimuth direction
noSubEl=round(probe.element_height/(lambda/1));       % number of subelements in the elevation direction
...
zzp_speckle=random('unif',0e-3,58e-3,number_of_scatterers,1);
